...
function message_amount($in) {
	global $link, $row,$tbl_messages,$gid,$current_time,$message_limit,$status_array;
$linkstart = '?'.$in;
if ($in == 'inbox') {
	$mid_rid = 'rid';
}else{
...
function message_inbox($mid) {
	global $link,$row,$tbl_messages,$gid,$current_time,$message_limit;
$linkstart = '?inbox';
...
function message_outbox($mid) {
	global $link,$row,$tbl_messages,$gid,$current_time,$message_limit,$db_main,$dbm_main,$tbl_members;
$linkstart = '?outbox';
...
$iquery="SELECT `id`,`Guild`,`Sex`,`Charname` FROM `$tbl_members` WHERE ($mids) ORDER BY `id` ASC LIMIT $message_limit";
...
function message_reply($mid) {
		global $link,$row,$tbl_messages,$fld_messages,$gid,$current_time,$current_date,$message;
