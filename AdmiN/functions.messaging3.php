...
function message_amount($in) {
	global $row,$tbl_messages,$gid,$current_time,$message_limit,$status_array;
$linkstart = '?'.$in;
if ($in == 'inbox') {
	$mid_rid = 'rid';
}else{
...
mysqli_query($link, "UPDATE `$tbl_messages` SET `status`='2',`mid`='$row->id', `rid`='' WHERE `id`='$mrow->id' LIMIT 1") or print(mysqli_error($link).'3');
...
function message_outbox($mid) {
	global $sid,$row,$tbl_messages,$gid,$current_time,$message_limit,$db_main,$dbm_main,$tbl_members;
...
$iquery="SELECT `id`,`sex`,`charname` FROM `$tbl_members` WHERE ($mids) ORDER BY `id` ASC LIMIT $message_limit";
...
mysqli_query($link, "UPDATE `$tbl_messages` SET `status`='4', `timer`='$current_time' WHERE `id`='$mrow->id' LIMIT 1") or print(mysqli_error($link).'4');
...
function message_create($mid) {
	global $row,$tbl_messages,$fld_messages,$gid,$current_time,$current_date,$message_limit,$alfa,$recipient_select;
...
$message .= $row->sex.' '.$row->charname.' <font size=-2>'.$row->class.' '.number_format($row->level).'</font>';

	mysqli_query($link, "INSERT INTO `$tbl_messages` ($fld_messages) VALUES ('NULL', '$gid', '$row->id', '$recipient', '$message', '0', '0', '$current_date', $current_time+$delivery_time, $current_time)") or print(mysqli_error($link));

	mysqli_query($link, "INSERT INTO `$tbl_messages` ($fld_messages) VALUES ('NULL', '$gid', '$row->id', '$recipient', '$message', '0', '1', '$current_date', $current_time+$delivery_time, $current_time)") or print(mysqli_error($link));
	
	//TEST SEND A COPY TO MYSELF
	//mysqli_query($link, "INSERT INTO `$tbl_messages` ($fld_messages) VALUES ('NULL', '$gid', '$recipient', '$row->id', '$message', '0', '0', '$current_date', $current_time+$delivery_time, $current_time)") or print(mysqli_error($link));
...
function message_reply($mid) {
		global $row,$tbl_messages,$fld_messages,$gid,$current_time,$current_date,$message;
...
print '<form method=post action="?reply&did='.$did.'" name=message_form>
...
$message .= $row->sex.' '.$row->charname.' <font size=-2>'.$row->class.' '.number_format($row->level).'</font>';
	...
	mysqli_query($link, "INSERT INTO `$tbl_messages` ($fld_messages) VALUES ('NULL', '$gid', '$row->id', '$mrow->mid', '$message', '0', '0', '$current_date', $current_time+$delivery_time, $current_time)") or print(mysqli_error($link));

	mysqli_query($link, "INSERT INTO `$tbl_messages` ($fld_messages) VALUES ('NULL', '$gid', '$row->id', '$mrow->mid', '$message', '0', '1', '$current_date', $current_time+$delivery_time, $current_time)") or print(mysqli_error($link));
	
	//TEST SEND A COPY TO MYSELF
	//mysqli_query($link, "INSERT INTO `$tbl_messages` ($fld_messages) VALUES ('NULL', '$gid', '$mrow->mid', '$row->id', '$message', '0', '0', '$current_date', $current_time+$delivery_time, $current_time)") or print(mysqli_error($link));
	print '<font color=red>Your message has been handed over to the postman, it will take a few seconds to deliver.</font>';
		}else{
			print '<font color=red>Postman has just left the building, please wait a few seconds before sending a message.</font>';
		}
...
function message_forward($mid) {
		global $row,$tbl_messages,$fld_messages,$gid,$current_time,$current_date,$message,$alfa,$recipient_select,$recipient,$message;
...
$message .= $row->sex.' '.$row->charname.' <font size=-2>'.$row->class.' '.number_format($row->level).'</font>';
	...
	mysqli_query($link, "INSERT INTO `$tbl_messages` ($fld_messages) VALUES ('NULL', '$gid', '$row->id', '$recipient', '$mrow->body<hr>$message', '0', '0', '$current_date', $current_time+$delivery_time, $current_time)") or print(mysqli_error($link));
	
	mysqli_query($link, "INSERT INTO `$tbl_messages` ($fld_messages) VALUES ('NULL', '$gid', '$row->id', '$recipient', '$mrow->body<hr>$message', '0', '1', '$current_date', $current_time+$delivery_time, $current_time)") or print(mysqli_error($link));
...
/*_______________-=TheSilenT.CoM=-_________________*/
require_once($_SERVER['DOCUMENT_ROOT'].'/AdmiN/array.emotions.php');
