...
?><table width=100%><tr><th colspan=2>Lords of Lords Hall of Fame<br>The best of best players from all servers!</th></tr><tr><th>Player</th><th>Level</th></tr><?php 
...
print '<tr><th colspan=4>The best of '.$key.'</th></tr>';

$presult = mysqli_query($link, "SELECT * FROM `$table_name` WHERE `id` ORDER BY `Level` DESC LIMIT $bestof");
...
if($key == 'Ysomite'){
$presult = mysqli_query($link, "SELECT * FROM `lol2_mstats` WHERE `id` ORDER BY `level` DESC LIMIT $bestof");
if($presult){
while ($srow = mysqli_fetch_object($presult)){
print '<tr><td>'.$no.'. '.$srow->charname.'</td><td>'.number_format($srow->level).'</td></tr>';
$no++;}
mysqli_free_result($presult);
}
}else{
