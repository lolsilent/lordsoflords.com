...
//print number_format($next_level).'<hr>';
$next_level = $next_level+($row->Level*pow($row->Level,2.25));
//print number_format($next_level).'<hr>';
...
//print 'POWER z'.$os[20];
$os[20] = $os[20] * (1+($bonus[14]/100));
//print 'POWER z'.$os[20];
$os[21]	= $os[10]/2.5;
$os[22]	= $os[11]/2.5;
$os[23]	= $row->Level;
$os[24]	= $races_array["$row->Race"][0]; // 12/02/17 04:28:44 added
$os[25]	= $races_array["$row->Race"][1];
$os[26]	= $races_array["$row->Race"][2];
$os[27]	= $races_array["$row->Race"][3];
$os[28]	= $races_array["$row->Race"][4];
...
function special($def,$opp,$wll,$divs) {
print "<font color=\"#888888\">";
$randdiv = array_rand ($divs, 2);
$def_mr = ($def[11]+$def[22]+$def[10]+$def[21]+$def[24])/$divs[$randdiv[0]];
$opp_mr = ($opp[11]+$opp[22]+$opp[10]+$opp[21]+$opp[24])/$divs[$randdiv[1]];
...
if ($def_mr >= $opp_mr) {
$randdiv = array_rand ($divs, 2);
$def_spell = ($def[2]+$def[3]+$def[0]+$def[1])/$divs[$randdiv[0]];
$m_shield = ($opp[6]+$opp[7]+$opp[8]+$opp[9])/$divs[$randdiv[1]];

	$def_spell-=$m_shield;
	if ($def_spell <= 0) {
		print "$wll[1] special abilities countered! ";
	} else {
		$opp[12]-=$def_spell;
		$def_spell = number_format($def_spell, 0, '', '.');
		$m_shield = number_format($m_shield, 0, '', '.');
print "$wll[0] special ability for $def_spell!<font size=-1>$wll[4] countered for $m_shield.</font>";
	}
} else {
	print "$wll[1] special abilities fails!";
}
print "</font><br>";
return $opp[12];
}
